...
class ArUcoDockingController(Node):
    def __init__(self):
        ...
        self.error_history = deque(maxlen=10)  # store last 10 filtered angle errors
        ...
        self.adaptive_alpha = 5.0  # scaling factor for the trend
    ...
    def timer_callback(self):
        ...
        if ids is not None:
            ...
            if best_tvec is not None:
                ...
                angle_error = self.normalized_angle_error(euler_angles_deg[1], 0.0)
    ...
    def process_docking(self, tvec, rvec, marker_id, twist, angle_error):
        ...
        if abs(filtered_angle) > tolerance_yaw:
            # 감속
            twist.linear.x *= 0.5
            
            angular_control = max(min(effective_Kp_y * filtered_angle * 0.5, 0.05), -0.05)
            ...
            if abs(tvec[0]) < tolerance_x and abs(tvec[2] - self.target_tvec_z) < tolerance_z:
                twist.linear.x = 0.0
                angular_control = 0.0
                ...
                angular_control = max(min(effective_Kp_y * filtered_angle * 0.3, 0.03), -0.03)
                # if abs(twist.linear.x) < 0.08:
                #     angular_control = 0.08 if angular_control > 0 else -0.08
        ...
        # self.prev_angular_z = angular_control
