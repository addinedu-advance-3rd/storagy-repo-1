...
class ArUcoDockingController(Node):
    ...
    def wheel_odom_callback(self, msg):
        ...
        if not self.odom_moving and self.odom_initial_pose is None:
            ...
            # self.get_logger().info(f"휠 오도메트리 이동거리: {self.odom_moving_distance:.8f}")
            twist = Twist()
            ...
            print("--------------------------------")
            print(f"ODOM MOVING DISTANCE : {self.odom_moving_distance:.8f}")
    ...
    def publish_docking_complete(self):
        ...
        self.get_logger().info("도킹 완료 메시지 게시")
        self.control_active = False
        self.get_logger().info("도킹 제어 비활성화")
    ...
    def timer_callback(self):
        ...
        print(f"twist.linear.x: {twist.linear.x:.2f}, twist.angular.z: {twist.angular.z:.2f}")
        print("--------------------------------")
        self.cmd_vel_pub.publish(twist) if not self.odom_control_active else print("ODOM CONTROL ACTIVE, DONT MOVE")
